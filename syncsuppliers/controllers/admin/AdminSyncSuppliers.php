...
class AdminSyncSuppliersController extends ModuleAdminController {
	...
	$query->join(' LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON c.`id_category` = cl.`id_category`');
	$query->where(
		($id_shop ? 'cl.`id_shop` = '.(int)$id_shop : '').' '.($id_lang ? 'AND `id_lang` = '.(int)$id_lang : '').'
		'.($active ? 'AND `active` = 1' : '').'
			  '.(!$id_lang ? 'GROUP BY c.id_category' : '')
	);
	$query->orderBy('c.`level_depth` ASC, c.`position` ASC');
